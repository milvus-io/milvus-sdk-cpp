...
UNITY=${UNITY:-OFF}
LINE=${LINE:-OFF}
MILVUS_UNITY_BATCH_SIZE=${MILVUS_UNITY_BATCH_SIZE:-8}
BUILD_FROM_CONAN="ON"
CPPSTD=${CPPSTD:-14}
if [[ ! "${CPPSTD}" =~ ^[0-9]+$ ]]; then
...
case "${UNITY}" in
  ON|OFF) ;;
  *)
    echo "ERROR! UNITY must be ON or OFF"
    exit 1
    ;;
esac
case "${LINE}" in
  ON|OFF) ;;
  *)
    echo "ERROR! LINE must be ON or OFF"
    exit 1
    ;;
esac
if ! [[ "${MILVUS_UNITY_BATCH_SIZE}" =~ ^[1-9][0-9]*$ ]]; then
  echo "ERROR! MILVUS_UNITY_BATCH_SIZE must be a positive integer"
  exit 1
fi
if [[ "${LINE}" == "ON" ]]; then
  MILVUS_DEBUG_INFO="line-tables"
else
  MILVUS_DEBUG_INFO="full"
fi
...
if [[ "${UNITY}" == "ON" && "${BUILD_COVERAGE}" == "ON" ]]; then
  echo "WARNING: UNITY=ON with coverage can distort lcov per-file line attribution (lines may be credited to the generated unity batch files). Keeping UNITY=OFF for coverage runs is recommended."
fi
...
if [ -z "${CMAKE_BUILD_EXAMPLES}" ]; then
    # Examples are excluded from coverage reports anyway (scripts/coverage.sh filters
    # "*/examples/*"), so skip compiling and linking 60+ instrumented binaries for
    # coverage runs unless explicitly requested.
...
MILVUS_ENABLE_PCH=ON
if [[ "${RUN_CPPLINT}" == "ON" ]]; then
  # clang-tidy is installed as a Clang binary and cannot read a PCH generated by GCC.
  MILVUS_ENABLE_PCH=OFF
  # Unity batches replace per-file entries in compile_commands.json, which breaks
  # clang-tidy's per-source lookups; lint trees must stay one file per TU.
  if [[ "${UNITY}" == "ON" ]]; then
    echo "NOTE: lint build forces UNITY=OFF so clang-tidy sees per-file compile commands."
    UNITY=OFF
  fi
fi
...
-DMILVUS_ENABLE_PCH=${MILVUS_ENABLE_PCH} \
${CMAKE_VERSION_ARG} \
-DCMAKE_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX} \
-DBUILD_SHARED_LIBS=${BUILD_SHARED_LIBS} \
-DMILVUS_ENABLE_UNITY=${UNITY} \
-DMILVUS_UNITY_BATCH_SIZE=${MILVUS_UNITY_BATCH_SIZE} \
-DMILVUS_DEBUG_INFO=${MILVUS_DEBUG_INFO} \
...
if [[ ${RUN_FORMAT} == "ON" ]]; then
  find ../src/include/milvus ../src/impl ../test ../examples ../tutorial \
    \( -iname '*.h' -o -iname '*.cpp' -o -iname '*.hpp' \) -print0 | xargs -0 make clang-format -i
fi
