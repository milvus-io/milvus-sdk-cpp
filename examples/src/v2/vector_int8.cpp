...
main(int argc, char* argv[]) {
    ...
    const std::string collection_name = "CPP_V2_INT8_VECTOR";
    ...
    collection_schema->AddField(
        milvus::FieldSchema(field_vector, milvus::DataType::INT8_VECTOR).WithDimension(dimension));
...

    {
        ...
        std::vector<milvus::FieldDataPtr> fields_data{
            ...
            std::make_shared<milvus::Int8VecFieldData>(field_vector, vectors)};
...

    {
        ...
        request.AddOutputField(field_vector);
        request.AddOutputField(field_text);
        request.AddInt8Vector(field_vector, q_vector_1.get<std::vector<int8_t>>());
        request.AddInt8Vector(field_vector, q_vector_2.get<std::vector<int8_t>>());
