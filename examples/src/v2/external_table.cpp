...
const char* const default_external_source = "s3://minio:9000/a-bucket/external_table_example_data/";
const nlohmann::json default_external_spec = {
    {"format", "parquet"},
    {"extfs",
...
waitRefreshComplete(milvus::MilvusClientV2Ptr& client, int64_t job_id) {
    for (int i = 0; i < 60; ++i) {
        ...
        std::cout << "Refresh job " << job_info.JobID() << ": state=" << std::to_string(job_info.State())
...
createExternalCollection(milvus::MilvusClientV2Ptr& client, const std::string& external_source) {
    ...
    schema->SetEnableDynamicField(false);  // external table doesn't support dynamic field
    schema->AddField(
        milvus::FieldSchema().WithName(field_id).WithDataType(milvus::DataType::INT64).WithExternalField("id"));
...
main(int argc, char* argv[]) {
    ...
    // Parent path to the existing Parquet files in the Milvus-configured object storage.
    // Each Parquet file is expected to contain source columns `id`, `vector`, and `text`,
    ...
    std::cout << "Collection external spec: " << desc_response.Desc().Schema().ExternalSpec().dump() << std::endl;
    ...
    std::cout << "Refresh completed: state=" << std::to_string(job_info.State()) << ", progress=" << job_info.Progress()
    ...
    for (const auto& job : jobs_response.Jobs()) {
        std::cout << "\tjobId=" << job.JobID() << ", collection=" << job.CollectionName()
                  << ", state=" << std::to_string(job.State()) << ", progress=" << job.Progress()
