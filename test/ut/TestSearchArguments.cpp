...
TEST_F(SearchArgumentsTest, VectorTesting) {
...

    {
        ...
        EXPECT_FALSE(status.IsOk());
        status = arguments.AddTargetVector<milvus::Float16VecFieldData>("dummy", new_vector);
        EXPECT_FALSE(status.IsOk());
        status = arguments.AddTargetVector<milvus::BFloat16VecFieldData>("dummy", new_vector);
        EXPECT_FALSE(status.IsOk());
    ...
    {
        milvus::SearchArguments arguments;
        auto status = arguments.AddTargetVector<milvus::Float16VecFieldData>("dummy", std::vector<float>{1.f, 2.f});
        EXPECT_TRUE(status.IsOk());

        std::vector<double> new_vector = {1.0, 2.0, 3.0};
        status = arguments.AddTargetVector<milvus::BFloat16VecFieldData>("dummy", new_vector);
        EXPECT_FALSE(status.IsOk());

        status = arguments.AddTargetVector<milvus::Float16VecFieldData>("dummy", std::vector<float>{1.f, 2.f, 3.f});
        EXPECT_FALSE(status.IsOk());

        status = arguments.AddTargetVector<milvus::Float16VecFieldData>("dummy", std::vector<float>{1.f, 2.f});
        EXPECT_TRUE(status.IsOk());

        auto target_vectors = arguments.TargetVectors();
        EXPECT_TRUE(target_vectors != nullptr);
        EXPECT_EQ(milvus::DataType::FLOAT16_VECTOR, target_vectors->Type());
        EXPECT_EQ(2, target_vectors->Count());
    }

    {
        milvus::SearchArguments arguments;
        auto status = arguments.AddTargetVector<milvus::BFloat16VecFieldData>("dummy", std::vector<float>{1.f, 2.f});
        EXPECT_TRUE(status.IsOk());
        ...
        status = arguments.AddTargetVector<milvus::BFloat16VecFieldData>("dummy", std::vector<float>{1.f, 2.f, 3.f});
        EXPECT_FALSE(status.IsOk());

        status = arguments.AddTargetVector<milvus::BFloat16VecFieldData>("dummy", std::vector<float>{1.f, 2.f});
        EXPECT_TRUE(status.IsOk());

        auto target_vectors = arguments.TargetVectors();
        EXPECT_TRUE(target_vectors != nullptr);
        EXPECT_EQ(milvus::DataType::BFLOAT16_VECTOR, target_vectors->Type());
        EXPECT_EQ(2, target_vectors->Count());
    }
}
