...
TEST_F(StatusTest, ConstructorWithErrorCodes) {
    milvus::Status status{milvus::StatusCode::RPC_FAILED, "rpc error", 14, 100, 200};
    EXPECT_FALSE(status.IsOk());
    EXPECT_EQ(status.Code(), milvus::StatusCode::RPC_FAILED);
    EXPECT_EQ(status.Message(), "rpc error");
    EXPECT_EQ(status.RpcErrCode(), 14);
    EXPECT_EQ(status.ServerCode(), 100);
    EXPECT_EQ(status.LegacyServerCode(), 200);
}

TEST_F(StatusTest, RpcErrCodeDefault) {
    milvus::Status status;
    EXPECT_EQ(status.RpcErrCode(), 0);
}

TEST_F(StatusTest, ServerCodeDefault) {
    milvus::Status status;
    EXPECT_EQ(status.ServerCode(), 0);
}

TEST_F(StatusTest, LegacyServerCodeDefault) {
    milvus::Status status;
    EXPECT_EQ(status.LegacyServerCode(), 0);
}