...
TEST_F(SearchResultsTest, RecallsAndWithRecalls) {
    milvus::SearchResults results;
    EXPECT_TRUE(results.Recalls().empty());

    std::vector<float> recalls = {0.95f, 0.88f, 0.92f};
    auto& ref = results.WithRecalls(std::move(recalls));
    EXPECT_EQ(ref.Recalls().size(), 3);
    EXPECT_FLOAT_EQ(ref.Recalls().at(0), 0.95f);
