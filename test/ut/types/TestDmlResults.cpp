...
TEST_F(DmlResultsTest, InsertCount) {
    milvus::DmlResults dml_results;
    EXPECT_EQ(dml_results.InsertCount(), 0);
    dml_results.SetInsertCount(100);
    EXPECT_EQ(dml_results.InsertCount(), 100);
}

TEST_F(DmlResultsTest, DeleteCount) {
    milvus::DmlResults dml_results;
    EXPECT_EQ(dml_results.DeleteCount(), 0);
    dml_results.SetDeleteCount(50);
    EXPECT_EQ(dml_results.DeleteCount(), 50);
}

TEST_F(DmlResultsTest, UpsertCount) {
    milvus::DmlResults dml_results;
    EXPECT_EQ(dml_results.UpsertCount(), 0);
