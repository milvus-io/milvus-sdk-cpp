...
TEST_F(MilvusMockedTest, AddPrivilegesToGroup) {
    ...
    EXPECT_CALL(service_, OperatePrivilegeGroup(_, _, _))
        .WillOnce([&group_name, &privileges](::grpc::ServerContext*, const OperatePrivilegeGroupRequest* request,
            ...
            EXPECT_EQ(request->type(), OperatePrivilegeGroupType::AddPrivilegesToGroup);
    ...
    auto status = client_->AddPrivilegesToGroup(group_name, privileges);
