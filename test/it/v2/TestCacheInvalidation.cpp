...
TEST_F(UnconnectMilvusMockedTest, V2FieldAndFunctionMutationsInvalidateCollectionSchema) {
    const auto endpoint = Endpoint(server_.ListenPort());
    auto client = CreateConnectedClient(service_, server_.ListenPort());

    EXPECT_CALL(service_, AddCollectionField(_, _, _))
        .WillOnce([](::grpc::ServerContext*, const milvus::proto::milvus::AddCollectionFieldRequest*,
                     milvus::proto::common::Status*) { return ::grpc::Status{}; });
    ...
    auto status = client->AddCollectionField(milvus::AddCollectionFieldRequest()
                                                 .WithDatabaseName("db")
                                                 .WithCollectionName("collection")
                                                 .WithField(milvus::FieldSchema("new_field", milvus::DataType::INT64)));
    EXPECT_TRUE(status.IsOk());
    ExpectNotCached(endpoint, "db", "collection");
