...
HeaderAdderInterceptor::HeaderAdderInterceptor(const std::unordered_map<std::string, std::string>& headers)
    : headers_(headers) {
}
...
HeaderAdderInterceptorFactory::CreateClientInterceptor(grpc::experimental::ClientRpcInfo* info) {
    return new HeaderAdderInterceptor(metadata_);
}
...
CreateChannelWithHeaderInterceptor(const std::string& target, const std::shared_ptr<grpc::ChannelCredentials>& creds,
    ...
    interceptor_creators.push_back(std::make_unique<HeaderAdderInterceptorFactory>(metadata));
