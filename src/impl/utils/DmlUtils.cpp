...
ProcessStructFieldValues(const EntityRow& row, const std::vector<StructFieldSchema>& struct_fields,
    ...
    for (const auto& struct_schema : struct_fields) {
        ...
        proto::schema::FieldData& field_data = proto_fields[name];
        std::map<std::string, proto::schema::FieldData*> proto_sub_fields;
        auto status = PrepareStructProtoFields(struct_schema, output_fields, field_data, proto_sub_fields);
        if (!status.IsOk()) {
            return status;
        }

        status = FillStructProtoFields(dict_list, struct_schema, output_fields, proto_sub_fields);
        if (!status.IsOk()) {
            return status;
        }
    }
...
PrepareStructProtoFields(const StructFieldSchema& schema, const std::set<std::string>& output_fields,
                         ...
                         std::map<std::string, proto::schema::FieldData*>& proto_sub_fields) {
    proto_sub_fields.clear();
    auto proto_struct_arrays = field_data.mutable_struct_arrays();
    for (const auto& sub_schema : schema.Fields()) {
        std::string sub_name = CombineStructFieldName(schema.Name(), sub_schema.Name());
        if (output_fields.find(sub_name) != output_fields.end()) {
            continue;
        }
        auto proto_sub_field = proto_struct_arrays->add_fields();
        proto_sub_field->set_field_name(sub_schema.Name());
        if (IsVectorType(sub_schema.FieldDataType())) {
            proto_sub_field->mutable_vectors();  // initialize it to be vector field
        } else {
            proto_sub_field->mutable_scalars();  // initialize it to be scalar field
        }
        proto_sub_fields.insert(std::make_pair(sub_schema.Name(), proto_sub_field));
    }
    return Status::OK();
}
...
FillStructProtoFields(const std::vector<nlohmann::json>& dict_list, const StructFieldSchema& schema,
                      const std::set<std::string>& output_fields,
                      std::map<std::string, proto::schema::FieldData*>& proto_sub_fields) {
    std::map<std::string, proto::schema::FieldData> temp_proto_fields;
    for (const auto& dict : dict_list) {
        auto status = ProcessNormalFieldValues(dict, schema.Fields(), output_fields, false, temp_proto_fields);
        if (!status.IsOk()) {
            return status;
        }
    }
    for (auto it : proto_sub_fields) {
        auto it_temp = temp_proto_fields.find(it.first);
        if (it_temp == temp_proto_fields.end()) {
            return {StatusCode::INVALID_AGUMENT, it.first + " is not found"};
        }
        auto proto_sub_field = it.second;
        if (proto_sub_field->has_scalars()) {
            auto scalars = (it_temp->second).scalars();
            proto_sub_field->mutable_scalars()->mutable_array_data()->add_data()->CopyFrom(scalars);
        } else if (proto_sub_field->has_vectors()) {
            auto vectors = (it_temp->second).vectors();
            proto_sub_field->mutable_vectors()->mutable_vector_array()->add_data()->CopyFrom(vectors);
        }
    }
    return Status::OK();
}
...
Status
CreateProtoStructData(const FieldDataPtr& column, const StructFieldSchema& schema,
    ...
    if (actual_data == nullptr) {
        return {StatusCode::INVALID_AGUMENT, "Illegal null pointer for field: " + schema.Name()};
    }

    std::map<std::string, proto::schema::FieldData*> proto_sub_fields;
    auto status = PrepareStructProtoFields(schema, output_fields, field_data, proto_sub_fields);
    if (!status.IsOk()) {
        return status;
    }
    ...
    for (const auto& dict_list : rows) {
        if (dict_list.size() > static_cast<std::size_t>(schema.MaxCapacity())) {
            std::string error_msg = "Array length " + std::to_string(dict_list.size()) + " exceeds max capacity " +
                                    std::to_string(schema.MaxCapacity()) + " of struct field: " + schema.Name();
            return {StatusCode::INVALID_AGUMENT, error_msg};
        }
        auto status = FillStructProtoFields(dict_list, schema, output_fields, proto_sub_fields);
        if (!status.IsOk()) {
            return status;
        }
    }
...
CheckAndSetRowData(const EntityRows& rows, const CollectionSchema& schema, bool is_upsert,
    ...
    // the proto_fields contains FieldData of normal fields and struct fields.
    ...
    for (const auto& struct_schema : struct_fields) {
        ...
        auto proto_sturct = fd.mutable_struct_arrays();
        for (const auto& sub_schema : struct_schema.Fields()) {
            auto sub_name = CombineStructFieldName(struct_schema.Name(), sub_schema.Name());
            auto it = proto_fields.find(sub_name);
            if (it != proto_fields.end()) {
                proto_sturct->mutable_fields()->AddAllocated(new proto::schema::FieldData(std::move(it->second)));
            }
        }
        rpc_fields.emplace_back(std::move(fd));
    }
