...
SearchRequest&
SearchRequest::WithOutputFieldNames(std::set<std::string>&& output_field_names) {
    SetOutputFieldNames(std::move(output_field_names));
    return *this;
}
...
int64_t
SearchRequest::GetRoundDecimal() const {
...
std::string
SearchRequest::GetGroupByField() const {
    return GetExtraStr(extra_params_, "group_by_field", "");
}

void
SearchRequest::SetGroupByField(const std::string& group_by_field) {
    SetExtraStr(extra_params_, "group_by_field", group_by_field);
}

SearchRequest&
SearchRequest::WithGroupByField(const std::string& group_by_field) {
    SetGroupByField(group_by_field);
    return *this;
}
