...
SearchRequestBase::EmbeddingLists() const {
    return embedding_lists_;
}
...
Status
SearchRequestBase::AddInt8Vector(const Int8VecFieldData::ElementT& vector) {
    return target_vectors_.AddInt8Vector(vector);
}
...
SearchRequestBase::AddEmbeddingList(EmbeddingList&& emb_list) {
    ...
    for (const auto& old_list : embedding_lists_) {
