...
MilvusClientImpl::SearchIterator(SearchIteratorArguments& arguments, SearchIteratorPtr& iterator) {
    ...
    auto ptrV2 =
        std::make_shared<SearchIteratorV2Impl>(connection_.GetConnection(), arguments, connection_.GetRetryParam());
    status = ptrV2->Init();
    iterator = ptrV2;
    if (!status.IsOk() && status.Code() == StatusCode::NOT_SUPPORTED) {
        auto ptrV1 =
            std::make_shared<SearchIteratorImpl>(connection_.GetConnection(), arguments, connection_.GetRetryParam());
        status = ptrV1->Init();
        if (!status.IsOk()) {
...
MilvusClientImpl::QueryIterator(QueryIteratorArguments& arguments, QueryIteratorPtr& iterator) {
    ...
    auto ptr = std::make_shared<QueryIteratorImpl>(connection_.GetConnection(), arguments, connection_.GetRetryParam());
    status = ptr->Init();
    if (!status.IsOk()) {
