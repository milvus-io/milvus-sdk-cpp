...
CreateProtoFieldData(const JsonFieldData& field) {
    auto ret = new proto::schema::ScalarField{};
    auto& data = field.Data();
    auto& scalars_data = *(ret->mutable_json_data()->mutable_data());
    for (const auto& item : data) {
        scalars_data.Add(item.dump());
    }
    return ret;
}
...
CreateMilvusFieldData(const milvus::proto::schema::FieldData& field_data, size_t offset, size_t count) {
    ...
    switch (field_type) {
        ...
        case proto::schema::DataType::JSON:
            return std::make_shared<JsonFieldData>(
                name, BuildJsonFieldDataScalars(field_data.scalars().json_data().data(), offset, count));
        default:
            return nullptr;
    }
}
...
CreateMilvusFieldData(const milvus::proto::schema::FieldData& field_data) {
    ...
    switch (field_type) {
        ...
        case proto::schema::DataType::JSON:
            return std::make_shared<JsonFieldData>(
                name, BuildJsonFieldDataScalars(field_data.scalars().json_data().data()));
        default:
            return nullptr;
    }
}
